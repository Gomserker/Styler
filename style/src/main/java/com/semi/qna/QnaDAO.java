...
public class QnaDAO {
	...
	private static final QnaDAO QNADAO = new QnaDAO();
	...
	public void getFreeDetail(HttpServletRequest request) {
		...
		ResultSet rs = null;
		String sql = "select * from postQ where p_no =?";
	...
	public void getNextFreeDetail(HttpServletRequest request) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "select * from postQ where p_date >(select p_date from postQ where p_no =?)and rownum = 1";
	...
	public void getPrevFreeDetail(HttpServletRequest request) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "select * from (select * from postQ order by p_date desc) "
				+ "	where p_date <(select p_date from postQ where p_no =?) and rownum =1";
	...
	public void writeFree(HttpServletRequest request) {
		...
		String sql = "insert into postQ values(post_seq.nextval, ?, ?, ?, sysdate, '코멘트t', ?, '0')";
	...
	public void getNoticeList(int page, HttpServletRequest request) {
		...
		String sql = "select * from ( select rownum num, P.* from (select * from postQ order by p_date desc) P)"
	...
	public void doSearch(int page, HttpServletRequest request) {
		...
		String sql = "select * from ( select rownum num, P.* from (select * from postQ where " + searchType
	...
	public void searchPaging(HttpServletRequest request) {
		...
		String sql = "select count(*) from postQ where " + searchType + " like '%'||?||'%' order by p_date desc";
	...
	public void upadateFreeDetail(HttpServletRequest request) {
		...
		// 날짜 date설정은
		// r_date=sysdate추가
		String sql = "update postQ set p_title=?, p_text=?, p_img=? where p_no=?";

		try {
			con = DBManager.connect();
			pstmt = con.prepareStatement(sql);

			String saveDirectory = request.getSession().getServletContext().getRealPath("qna/img");
			System.out.println(saveDirectory);
	...
	public void deleteFreeDetail(HttpServletRequest request) {
		...
		// 날짜 date설정은
		// r_date=sysdate추가
		String sql = "delete postQ where p_no=?";
	...
	public void countView(HttpServletRequest request) {
		...
		String sql = "update postQ set p_view_count = p_view_count + 1  where p_no=?";
