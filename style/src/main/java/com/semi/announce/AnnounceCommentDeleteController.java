...
@WebServlet("/AnnounceCommentDeleteController")
public class AnnounceCommentDeleteController extends HttpServlet {
	
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//		FreeDAO.getFreedao().deleteComment(request);
		AnnounceDAO.getAnouncedao().deleteComment(request);
//		FreeDAO.getFreedao().countView(request);
		AnnounceDAO.getAnouncedao().countView(request);
