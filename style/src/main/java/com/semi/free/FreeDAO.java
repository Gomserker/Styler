...
public class FreeDAO {
	...
	private static final FreeDAO FREEDAO = new FreeDAO();

	public FreeDAO() {
	}

	public static FreeDAO getFreedao() {
		return FREEDAO;
	}
	 ...
	 * 
	 * try { con = DBManager.connect(); pstmt = con.prepareStatement(sql); rs =
	 * pstmt.executeQuery();
	 * 
	 * ArrayList<Post> posts = new ArrayList<Post>(); Post p = null; while
	 * (rs.next()) { p = new Post(); p.setP_no(rs.getString("p_no"));
	 * p.setP_title(rs.getString("p_title")); p.setP_user(rs.getString("p_user"));
	...
	public void doSearch(int page, HttpServletRequest request) {
		...
		String sql = "select * from ( select rownum num, P.* from (select * from post where " + searchType
